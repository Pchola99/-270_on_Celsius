...
public class Physics extends Thread {
    ...
    public void run() {
        while (!glfwWindowShouldClose(glfwWindow)) {
            ...
            if (DynamicObjects[0].player && EventHandler.getKey(GLFW_KEY_SPACE)) jump();
            if (DynamicObjects[0].player && EventHandler.getKey(GLFW_KEY_D) || EventHandler.getKey(GLFW_KEY_A)) move();
        }
    }
