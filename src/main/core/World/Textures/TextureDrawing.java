...
public class TextureDrawing {
    ...
    public static void updateDynamicObj() {
        try (var ignored = batch.pushState()) {
                ...
                if (dynamicObject != null) {
                    ...
                    if (isOnCamera(dynamicObject.getX(), dynamicObject.getY(), dynamicObject.getTexture())) {
                        if (dynamicObject.getFramesCount() == 0) {
                            SimpleColor color = ShadowMap.getColorDynamic(dynamicObject);
                            batch.draw(dynamicObject.getTexture(), color, dynamicObject.getX(), dynamicObject.getY());
                        } else {
                            // todo дописать
